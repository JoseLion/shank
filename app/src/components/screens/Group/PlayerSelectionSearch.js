...
export default class PlayerSelectionSearch extends BaseComponent {
    ...
    searchInPlayers(playerName) {
        ...
        let found = this.state.players.filter(player => {
            return player.FirstName.toUpperCase().indexOf(playerName.toUpperCase().trim()) !== -1
                || player.LastName.toUpperCase().indexOf(playerName.toUpperCase().trim()) !== -1
                || player.FirstName.concat(' ').concat(player.LastName).toUpperCase().indexOf(playerName.toUpperCase().trim()) !== -1;
        });

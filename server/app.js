...
mongoose.connect(databaseUri, {}).then(async () => {
	...
	let count = 0;
	await tournaments.forEach(async tournament => {
		let leaderboard = await fantasy.updateLeaderboard(tournament.tournamentID).catch(error => console.log("Error fetching from fantasydata.net: ", error));
		count += leaderboard.length;
	});
	console.log("Updated " + count + " leaderboards in " + tournaments.length + " tournaments!");

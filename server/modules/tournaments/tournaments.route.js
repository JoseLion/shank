	...
	.post(auth, multer().fields([{ name: 'file1', maxCount: 1 }, { name: 'file2', maxCount: 1 }]), (req, res) => {
		try {
			...
			Q.all(promises).spread((archive1_saved, archive2_saved, tournament_saved) => {
				...
				const remindTime = `${remindDate.getUTCSeconds()} ${remindDate.getUTCMinutes()} ${remindDate.getUTCHours()} ${remindDate.getUTCDate()} ${remindDate.getMonth() + 1} ${remindDate.getUTCDay()}`;
				CronJobs.create({
                    cronTime: remindTime,
				...
				const beginTime = `${beginDate.getUTCSeconds()} ${beginDate.getUTCMinutes()} ${beginDate.getUTCHours()} ${beginDate.getUTCDate()} ${beginDate.getMonth() + 1} ${beginDate.getUTCDay()}`;
                CronJobs.create({
                    cronTime: beginTime,
                ...
                tournament_saved.rounds.forEach(round => {
                    ...
                    const cronTime = `${endDate.getUTCSeconds()} ${endDate.getUTCMinutes()} ${endDate.getUTCHours()} ${day.getUTCDate()} ${day.getMonth() + 1} ${day.getUTCDay()}`;
